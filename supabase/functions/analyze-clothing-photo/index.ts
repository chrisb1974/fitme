// Analyze a clothing photo: Google Vision (labels/colors/logos) + Claude structuring.
// Ported from the Base44 backend function; InvokeLLM -> direct Anthropic call.
...
function normalize(value: string, list: string[]) {
  if (!value) return '';
...
async function callGoogleVision(base64Data: string) {
  ...
  if (!res.ok) {
    console.error('Google Vision error', res.status, (await res.text()).slice(0, 300));
    return null;
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 1. Google Vision (optional, if base64 provided and key configured)
    let hasVision = false;
    let visionPrompt = '';
    let detectedBrand = '';
    ...
    if (base64Image) {
      ...
      if (v) {
        hasVision = true;
        const labels = (v.labelAnnotations || []).map((l: { description: string; score: number }) => l.description + '(' + Math.round(l.score * 100) + '%)').join(', ');
        const objects = (v.localizedObjectAnnotations || []).map((o: { name: string }) => o.name).join(', ');
        const logos = (v.logoAnnotations || []).map((l: { description: string }) => l.description).join(', ');
    ...
    // 2. Claude
    const LANG_NAMES: Record<string, string> = { en: 'English', fr: 'French', es: 'Spanish' };
...
- low_confidence: true ONLY if the image is very unclear or item completely unidentifiable`;
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        const res = await invokeClaude({
          prompt,
          // Prefer inline base64 (robust); fall back to the URL if that's all we have.
          images_base64: base64Image ? [base64Image] : undefined,
          file_urls: !base64Image && photoUrl ? [photoUrl] : undefined,
          model: defaultModel(),
          response_json_schema: schema,
        });
        object = (res.object as Record<string, unknown>) || null;
        if (object) break;
      } catch (err) {
    ...
    const r = object || {};
    const seasons = Array.isArray(r.seasons) ? r.seasons as string[] : [];

    return json({
      name: (r.name as string) || '',
      category: normalize(r.category as string, CATEGORIES),
      color: normalize(r.color as string, COLORS),
      brand: (r.brand && r.brand !== 'Unknown') ? (r.brand as string) : (detectedBrand || ''),
      season: seasons.length > 0 ? seasons : ['Spring', 'Summer', 'Autumn', 'Winter'],
    ...
    return json({ error: (err as Error).message, low_confidence: true }, 500);
  }
});
